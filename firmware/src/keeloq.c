...
/* ---- per-fob device key derivation (Normal learning) ------------------ */
static uint64_t derive_device_key(uint32_t serial)
{
...
bool keeloq_decode(uint64_t raw_frame, uint8_t nbits, keeloq_frame_t *out)
{
    ...
    uint64_t dkey = derive_device_key(serial);
    uint32_t dec  = keeloq_decrypt(hop, dkey);
